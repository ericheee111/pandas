...
"""Self-contained 920b aggregate benchmarks for reshape."""
...
# ---- Inlined official source: reshape ----
...
class _Official_reshape_Melt:
    params = ['float64', 'Float64']
    param_names = ['dtype']

    def setup(self, dtype):
        self.df = DataFrame(np.random.randn(100000, 3), columns=['A', 'B', 'C'], dtype=dtype)
...
class _Official_reshape_SimpleReshape:
...
class _Official_reshape_ReshapeExtensionDtype:
...
class _Official_reshape_ReshapeMaskedArrayDtype(_Official_reshape_ReshapeExtensionDtype):
    ...
    def setup(self, dtype):
        ...
        ri = pd.Index(range(1000))
        mi = MultiIndex.from_product([lev, ri], names=['foo', 'bar'])
        values = np.random.randn(10000).astype(int)
        ser = pd.Series(values, dtype=dtype, index=mi)
        df = ser.unstack('bar')
        self.ser = ser
        self.df = df

class _Official_reshape_Unstack:
    params = ['int', 'category']
...
class _Official_reshape_SparseIndex:
...
class _Official_reshape_WideToLong:
...
class _Official_reshape_PivotTable:
    ...
    def time_pivot_table_categorical(self):
        self.df2.pivot_table(index='col1', values='col3', columns='col2', aggfunc='sum', fill_value=0)

    def time_pivot_table_categorical_observed(self):
        self.df2.pivot_table(index='col1', values='col3', columns='col2', aggfunc='sum', fill_value=0, observed=True)

    def time_pivot_table_margins_only_column(self):
        self.df.pivot_table(columns=['key1', 'key2', 'key3'], margins=True)

class _Official_reshape_Crosstab:
...
class _Official_reshape_GetDummies:

    def setup(self):
        categories = list(string.ascii_letters[:12])
        s = pd.Series(np.random.choice(categories, size=1000000), dtype=CategoricalDtype(categories))
        self.s = s
...
class _Official_reshape_Explode:
...
class Crosstab(_AggregateBenchmark):
    ...
    case_params = (
        _select_case_params(_Official_reshape_Crosstab, 'time_crosstab', ()),  # reshape.Crosstab.time_crosstab
        _select_case_params(_Official_reshape_Crosstab, 'time_crosstab_normalize', ()),  # reshape.Crosstab.time_crosstab_normalize
        _select_case_params(_Official_reshape_Crosstab, 'time_crosstab_normalize_margins', ()),  # reshape.Crosstab.time_crosstab_normalize_margins
        _select_case_params(_Official_reshape_Crosstab, 'time_crosstab_values', ()),  # reshape.Crosstab.time_crosstab_values
    )
    run_repeat = (
        2,  # reshape.Crosstab.time_crosstab
        2,  # reshape.Crosstab.time_crosstab_normalize
        1,  # reshape.Crosstab.time_crosstab_normalize_margins
        2,  # reshape.Crosstab.time_crosstab_values
    )
    case_methods = (
        'time_crosstab',
...
class PivotTable(_AggregateBenchmark):
    ...
    case_params = (
        ...
        _select_case_params(_Official_reshape_PivotTable, 'time_pivot_table_categorical_observed', ()),  # reshape.PivotTable.time_pivot_table_categorical_observed
        _select_case_params(_Official_reshape_PivotTable, 'time_pivot_table_margins', ()),  # reshape.PivotTable.time_pivot_table_margins
