    ...
    with nogil:
        ...
        for i in range(N):
            ...
            if i == 0:
                ...
                sum_x += values[e - 1]
                for j in range(max(0, i - window_size), s):
                    sum_x -= values[j]
...
def roll_mean_fixed_no_nan_int64(const int64_t[:] values,
    ...
    with nogil:
        ...
        for i in range(N):
            ...
            if (e - s) >= minp:
                output[i] = <float64_t>sum_x / <float64_t>(e - s)
            else:
                output[i] = NaN
...
def roll_std_fixed_no_nan_int64(const int64_t[:] values,
    ...
    cdef:
        Py_ssize_t i, j, N = len(values)
        int64_t sum_x, sum_sq, val, s, e, nobs
    ...
    with nogil:
        ...
        for i in range(N):
            ...
            if nobs >= minp and nobs > ddof:
                mean_val = <float64_t>sum_x / <float64_t>nobs
                var_val = <float64_t>sum_sq / <float64_t>nobs - mean_val * mean_val
                if var_val < 0:
                    var_val = 0
                output[i] = sqrt(var_val * <float64_t>nobs / (<float64_t>nobs - <float64_t>ddof))
            else:
