...
def test_factorize_int64_dense_range_basic():
    # Dense int64 keys, shuffled, 0-based start.
    rng = np.random.RandomState(0)
    values = np.ascontiguousarray(
        rng.randint(0, 500, size=200_000).astype(np.int64)
    )
    result = ht.factorize_int64_dense_range(values)
    assert result is not None
    labels, uniques = result

    # uniques sorted ascending
    assert (uniques[1:] > uniques[:-1]).all()
    # labels are a valid indexer into uniques
    reconstructed = uniques[labels]
    tm.assert_numpy_array_equal(reconstructed, values)
    # dtype contract
    assert labels.dtype == np.dtype(np.intp)
    assert uniques.dtype == np.dtype(np.int64)


def test_factorize_int64_dense_range_negative_and_gaps():
    rng = np.random.RandomState(1)
    # Even numbers in [-200, 200) → gaps at odd values, includes negatives.
    values = np.ascontiguousarray(
        (rng.randint(-100, 100, size=200_000) * 2).astype(np.int64)
    )
    result = ht.factorize_int64_dense_range(values)
    assert result is not None
    labels, uniques = result
...
def test_factorize_int64_dense_range_fallbacks():
    # Short input → None
    assert ht.factorize_int64_dense_range(np.arange(1000, dtype=np.int64)) is None

    # Large sparse span → None
    rng = np.random.RandomState(2)
    sparse = np.ascontiguousarray(
        rng.randint(-(2**62), 2**62, size=200_000).astype(np.int64)
    )
    assert ht.factorize_int64_dense_range(sparse) is None

    # INT64_MIN..INT64_MAX extremes → overflow-safe None (no crash)
    extremes = np.empty(200_000, dtype=np.int64)
    extremes[: 100_000] = np.iinfo(np.int64).min
    extremes[100_000 :] = np.iinfo(np.int64).max
    assert ht.factorize_int64_dense_range(extremes) is None
